...
def format_to_dataframe(data: Iterable[JSONDataType]) -> pd.DataFrame:
    """
    Processes scraped JSON data, extracts details and reformats to dataframe.
    """
    listings = []
    for entry in tqdm(data, desc="Processing html content..."):
        details_to_extract = _extract_property_details(entry)
        listings.append(details_to_extract)
...
def _extract_property_details(entry: JSONDataType) -> JSONDataType:
    """
    Extracts property details from a scraped JSON data entry.
    """
    ...
    def get_previous_sales(property_details: JSONDataType) -> JSONDataType:
        previous_sales = []
        if isinstance(property_details, dict) and "salesOfResidence" in property_details:
            for sale in property_details["salesOfResidence"]:
                previous_sales.append(int(sale["booliId"]))
        return {
            "booli_id_of_previous_sales": previous_sales,
    ...
    return (
        ...
        | {
            ...
            "agency_id": property_details.get("agencyId"),  # type: ignore
            "agent_id": property_details.get("agentId"),  # type: ignore
        }
